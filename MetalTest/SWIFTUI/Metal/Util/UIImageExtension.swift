...
public extension UIView {
    func createUIImage() -> UIImage? {
        UIGraphicsBeginImageContextWithOptions(
               CGSize(width: self.frame.width, height: self.frame.height), false, UIScreen.main.scale)
...
public extension UIImage {
    ...
    //center insider  保证image不会超过width 和height
    //如果image小，不放大，如果image大，则缩放
    func centerInside(width: Double, height: Double, opaque: Bool = false) -> UIImage? {
        if size.width <= width && size.height <= height {
            return self
        }
        if size.width / width > size.height / height {
            return scaled(toWidth: width, opaque: false)
        } else {
            return scaled(toHeight: height, opaque: false)
        }
    }
...
public extension UIImage {
    ...
    /// 切图，以aspect fill的形式
    /// - Parameter targetSize: 要切的大小
    /// - Returns: 图片
    func resizeImageToAspectFill(targetSize: CGSize) -> UIImage? {
        ...
        let originAspectRatio = self.size.width / self.size.height
        let aspectRatio = width / height
        var newWidth: CGFloat = width
        var newHeight: CGFloat = height

        if aspectRatio >= originAspectRatio { //
            newHeight = newWidth / originAspectRatio
        } else {
            newWidth = newHeight * originAspectRatio
        }
        ...
        if let currentImage = currentImage {
            let rect = CGRect(x: floor((currentImage.size.width - width) / 2), // 取整，避免切出的size不对，下同
                   y: floor((currentImage.size.height - height) / 2),
                   width: width,
                   height: height)
            if let currentCGImage = currentImage.cgImage,
               let cropCGImage = currentCGImage.cropping(to: rect) {
